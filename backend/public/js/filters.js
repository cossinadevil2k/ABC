(function($a) {
	...
	$a.module('ML.filters', [])
		...
		.filter('dateTimeFromNow', function(){
		...
		.filter('checkDateTimeSeconds', function(){
			return function(value){
				var date = moment(value);
				if(date.toISOString()===value){
		...
		.filter('checkDateTimeDateOnly', function(){
			return function(value){
				var date = moment(value);
				if(date.toISOString()===value){
		...
		.filter('countryCode', function() {
			return function(code) {
				...
				return COUNTRY_MAP[code.toUpperCase()] || 'Unknown';
			}
		});
}(angular));
