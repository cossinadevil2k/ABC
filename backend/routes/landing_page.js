...
let LandingStatic = mongoose.model('LandingStatic')
...
//         });
// }
let appLandingCreate = function(req,res){
    ...
    LandingStatic.addNew({
        userId: adminId,
        subDomain: data.subdomain + '.' + data.domain,
        domain: data.domain,
        url: data.url,
        status: data.status,
...
let appBrowseLanding = function(req,res){

    LandingStatic.getAll(function(error, result){
        if(error){
            return res.json({
                status: false,
                message: error
...
module.exports = function(app,config){
    app.get('/landing', staticsMain);
    app.post('/api/landing_page/create',appLandingCreate);
    app.post('/api/landing_page/loadbrowse',appBrowseLanding);
    //app.use(checkAdmin);
}