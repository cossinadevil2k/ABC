...
function findUserExpireWork(callback) {
    ...
    let startDate = moment().startOf('days').subtract('1', 'day').toDate();
    let endDate = moment().endOf('days').subtract('1', 'day').toDate();
    let condition;
...
function mainFunc() {
    ...
    let listUserRenewed = [];
    async.series({
        connectDBTask: function (callback) {
            connectDB(callback);
        },
                                    ...
                                    if (error) {
                                        ...
                                        if (resource) {
                                            ...
                                            if (resource.cancelReason === 0 || resource.cancelReason === 1) {
                                                ...
                                                RawDataPublisher.publish(EVENT_NAME, purchaseInfo, Rabbit.PRIORITY.critical, next);
                                                // next();
                                            } else if (resource.autoRenewing) {
                                                // auto renew successfully
                                                isRenew = true;
                                                let expire_time = resource.expiryTimeMillis;
                                                /**
                                                   * 1-Payment received
                                                   *  2-Free trial
                                                   */
