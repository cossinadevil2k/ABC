...
const SELECT_USER_COLLECTION = '_id createdDate lastSync tags purchased premium_at isDeactivated rwExpire tags';
...
// let startDay = moment().startOf('day').subtract(8, 'days').toDate();
// let endDay = moment().startOf('day').subtract(1, 'days').toDate();
...
function getSaleLogRecord(callback) {
    ...
    let startDay = "2017-12-08T00:00:00.000Z";
    let endDay = "2017-12-15T00:00:00.000Z";

    startDay = moment(startDay).toDate();
...
function putDiscountTrackingRecordIntoQueue(discountRecords) {
    return new Promise(function (resolve, reject) {
        discountRecords.forEach(function (item, index, collection) {
            setTimeout(function () {
                // console.time('excuteTime');

                RawDataPublisher.publish(EVENT_DISCOUNT_TRACKING, item, Rabbit.PRIORITY.normal);
                // console.log('index ', index);
                // console.timeEnd('excuteTime');
...
function getDiscountTrackingRecord(callback) {
    let startDay = moment().startOf('day').subtract(1, 'days').toDate();
    let endDay = moment().startOf('day').toDate();
    ...
    DiscountTrackingModel.find(condition)
        .lean(true)
        .exec(callback);
}


function discountTrackingMain() {
    async.waterfall([
        function (callback) {
            getDiscountTrackingRecord(callback);
        },
        function (discountRecords, callback) {
            if (discountRecords.lenght <= 0) {
                callback();
            } else {
                putDiscountTrackingRecordIntoQueue(discountRecords)
                    .then(function (data) {
                        callback(null, discountRecords);
                    })
                    .catch(function (error) {
                        callback(error, null);
                    })
...
let discountTrackingTask = new CronJob({
    cronTime: SCHEDULE_BUS['sale'],
    onTick: discountTrackingMain,
    start: false,
    timeZone: 'Asia/Ho_Chi_Minh'
})
